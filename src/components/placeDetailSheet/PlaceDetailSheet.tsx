...
export default function PlaceDetailSheet() {
    ...
    >({
        ...
        queryFn: async () => {
            if (!selectedMarker)
                throw new Error("Marker가 선택되지 않았습니다.");
    ...
    const currentSlot = chartData.find(
        (c: { timeRange: string; db: number; count: number }) =>
            c.timeRange === currentRange,
    );
    const avgDecibel = currentSlot?.db ?? data?.avgDecibelCached ?? 0;
                    ...
                    {!isLoading && data && (
                        <>
                            ...
                            <article className="flex-1 min-h-0 flex flex-col">
                                <h4 className="mb-3 text-base !font-bold text-neutral-black">
                                    한줄평
                                </h4>
